...
resource "aws_s3_bucket_object" "index_html" {
  bucket       = var.bucket_name
  key          = "index.html"
  source       = "~/my-cv-deployment-project.tf/cv-files/index.html"
  content_type = "text/html"

  # The filemd5() function is available in Terraform 0.11.12 and later
  # For Terraform 0.11.11 and earlier, use the md5() function and the file() fu>
  # etag = "${md5(file("path/to/file"))}"
  etag = filemd5("~/my-cv-deployment-project.tf/cv-files/index.html")
}

resource "aws_s3_bucket_object" "index_css" {
  bucket       = var.bucket_name
  key          = "index.css"
  source       = "~/my-cv-deployment-project.tf/cv-files/index.css"
  ...
  # For Terraform 0.11.11 and earlier, use the md5() function and the file() fu>
  # etag = "${md5(file("path/to/file"))}"
  etag = filemd5("~/my-cv-deployment-project.tf/cv-files/index.css")
}

resource "aws_s3_bucket_object" "error_html" {
  bucket       = var.bucket_name
  key          = "error.html"
  source       = "~/my-cv-deployment-project.tf/cv-files/error.html"
  content_type = "text/html"

  # The filemd5() function is available in Terraform 0.11.12 and later
  # For Terraform 0.11.11 and earlier, use the md5() function and the file() fu>
  # etag = "${md5(file("path/to/file"))}"
  etag = filemd5("~/my-cv-deployment-project.tf/cv-files/error.html")
}
...
resource "aws_route53_record" "www" {
  ...
  alias {
    name                   = aws_cloudfront_distribution.s3_distribution.domain>
    zone_id                = aws_cloudfront_distribution.s3_distribution.hosted>
...
resource "aws_cloudfront_distribution" "s3_distribution" {
  origin {
    ...
    s3_origin_config {
      origin_access_identity = "origin-access-identity/cloudfront/${aws_cloudfr>
    }
  }
